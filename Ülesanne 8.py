...
varem sihtpunkti. Kui palju me aga ajas võidame?
Koostage programm, mis küsib käivitamisel läbitava vahemaa pikkust (kilomeetrites,
see kehtib kogu programmitöö aja), seejärel aga küsib kasutajalt korduvalt
kiirust (km/h) ning väljastab selle põhjal korrektse lausega sõiduks kuluva aja ja
erinevuse eelmisest tulemusest. Kui kasutaja uut kiirust ei sisesta, vaid vajutab
...
if __name__ == '__main__':
    Vahemaa = int(input("Sisesta läbitav vahemaa kilomeetrites: "))
    kiirus_text = "1"
    previous_travel_time = -1
    while len(kiirus_text) > 0:
        kiirus_text = input("Sisesta sõidukiirus: ")
        if kiirus_text.isdigit():
            speed = int(kiirus_text)
            travel_time = calculate_travel_time_in_seconds(Vahemaa, speed)
